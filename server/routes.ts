...
export async function registerRoutes(
  ...
  app.post("/api/jobs/ingest", async (req, res) => {
    try {
      ...
      // Log activity (before async matching to ensure it's logged)
      const { activityLogger } = await import("./logger");
      ...
      // Send response immediately to prevent n8n timeout
      // Auto-matching will continue asynchronously in the background
      res.json({
        ok: true,
        processed,
        inserted,
        updated,
        skipped,
        matchingQueued: jobsToMatch.length, // Inform n8n that matching is queued
      });
