...
export default function Dashboard() {
  ...
  const { data: allJobs = [], isLoading: jobsLoading } = useQuery({
    ...
    queryFn: () => {
      ...
      // If rejected filter is active, filter by rejected status
      if (rejectedFilter) {
            ...
            <>
              ...
              >
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  ...
                  <XCircle className={`h-4 w-4 ${rejectedFilter ? "text-destructive" : "text-primary"}`} />
                </CardHeader>
                <CardContent>
                  <div className={`text-2xl font-bold font-mono ${rejectedFilter ? "text-destructive" : ""}`}>
                    {stats.rejectedJobs || 0}
                  </div>
                  <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                    {rejectedFilter ? (
                      <span className="text-destructive font-medium">Showing rejected jobs</span>
                    ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          ...
          <div className="lg:col-span-2 space-y-4 md:space-y-6 min-w-0">
            ...
            {jobsLoading ? (
              ...
              jobs.length === 0 ? (
                <div className="text-center py-12">
                  <p className="text-muted-foreground">No rejected jobs found.</p>
                </div>
              ) : (
                <div className="grid gap-4 animate-in fade-in slide-in-from-bottom-8 duration-700 fill-mode-forwards">
                  {jobs.map((job) => (
